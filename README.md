...
| Concurrent requests | Number of requests sent to translation providers at the same time (1-8); raise it to speed up long pages | 3 |
| Default display | Translation, source, or bilingual | Translation |
| Image text OCR | Run OCR on page images and display translated image text | Enabled |
| Maximum images per page | Maximum image candidates processed per page; the rest are reported as skipped | 30 |
| Minimum image edge | Edge-size threshold in pixels used to filter icons, avatars, and tracking pixels | 64 |
| Image translation display | Show a caption below the image, or overlay a translucent translation inside the image | Below image |

Long paragraphs are split at sentence boundaries and then by safe character boundaries when necessary. Provider requests are never silently truncated. If the page exceeds the block or character limits, or if individual text units fail, the top status area clearly reports that only part of the page was translated. Completed page translations are reused through bounded in-memory LRU caches: up to 30 page entries with up to 3 language directions per page, plus a shared translation-result cache bounded at 50,000 entries or approximately 50 MB. These caches are memory-only and are not written to disk. Cancelling, navigating, re-extracting, changing languages, or closing the reader invalidates the previous job so late provider responses cannot overwrite the current page. In-place updates only assign matching `TextNode.nodeValue` values, preserving DOM structure, links, buttons, event handlers, scroll position, and surrounding whitespace. Anchor mismatches are skipped and reported as a page update. Bilingual mode renders translations through controlled injected nodes; the extraction script and incremental collector skip these nodes so translations are never re-translated as source text. A paragraph is only rendered when every text unit in its block has a successful translation; failed or mismatched blocks stay in the source language and are reported separately for retry. Buttons, navigation, form semantic hints, and short fragments keep their source text in bilingual mode to avoid stretching the layout; they are still translated and cached, so switching to **Translation** shows their translations immediately. Text blocks inside open Shadow DOM are not supported in bilingual mode and are counted separately as intentionally skipped rather than failures.

When **Image text OCR** is enabled, clicking **Translate this page** collects visible image candidates (`img`, `picture`, `canvas`, and sized elements with a `background-image`) alongside the text snapshot. Hidden images, decorative images, and images smaller than **Minimum image edge** are filtered out, and at most **Maximum images per page** images are processed. Image bytes are fetched through the reader's isolated session with the page cookies; if the request fails, is cross-origin restricted, or the source is a `canvas`, `data:`, or `blob:` inline resource, the reader falls back to capturing that image region from the remote page. Fetching is bounded to 8 MB and 16 megapixels per image; oversized images are reported as skipped and never block text translation.

Image bytes are processed locally by the existing OCR engines, reusing the OCR language, engine preference, adaptive preprocessing, and quality gate. Empty, noisy, or low-quality OCR results are never translated or rendered. Translations appear as a caption below the image by default, or as a translucent overlay inside the image when **Overlay on image** is selected; overlay nodes use theme variables, never rewrite image bytes, and never remove or replace the original image element. Image OCR and translation results are cached per page fingerprint, image fingerprint, and language direction in a partition separate from text, so retranslating the same page in the same direction reuses them without fetching or running OCR again. Failed or cancelled results are not cached. The status bar reports processed/candidate, skipped, and failed image counts independently, and image failures are surfaced as a partial-translation notice without affecting text translation completion.

Web page translation runs provider requests with 3-way concurrency by default and can be tuned from 1 to 8 through **Concurrent requests** in settings. Page write-back is decoupled from network requests: as soon as a text unit is translated it enters an in-order write-back queue, so slow DOM updates no longer occupy a network concurrency slot and later paragraphs keep being requested. Higher concurrency increases instantaneous provider load and may trigger rate limiting or circuit breaking, so prefer higher values with self-hosted or high-quota providers.
...
- Extraction reads only the currently rendered and visible DOM in the main document, including visible image candidates when image text OCR is enabled. It does not cross into iframes, traverse Shadow DOM, read CSS pseudo-elements, or scroll the page to trigger lazy loading;
- Extraction is read-only. The separate controlled apply script only changes matching text-node values and never rewrites image bytes; image fetching uses the reader's isolated session and is skipped entirely when image text OCR is disabled;
- Extracted text and OCR-recognized image text are sent only through the translation providers currently enabled by the user; image bytes stay local and are never uploaded as images. Review those providers' privacy and data-retention terms before translating sensitive pages.
