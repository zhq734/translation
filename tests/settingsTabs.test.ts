...
const TAB_IDS = ['general', 'ai', 'ocr', 'dingtalk', 'microsoft', 'deeplx', 'advanced', 'about'] as const
...
test('设置页应按八个配置类别展示可访问的 Tab 与对应面板', () => {
...
test('设置项应分配到常规、翻译服务、DeepLX 和高级配置面板', () => {
  ...
  const ocrIndex = html.indexOf('id="settings-panel-ocr"')
  ...
  assert.ok(ocrIndex > aiIndex)
  assert.ok(dingTalkIndex > ocrIndex)
  ...
  assert.ok(html.indexOf('<h2>OCR 翻译</h2>') > ocrIndex)
  assert.ok(html.indexOf('<h2>模型资产</h2>') > ocrIndex)
