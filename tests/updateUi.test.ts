...
test('共享类型和预加载层应只暴露受限的自动更新接口', () => {
  ...
  assert.match(preloadSource, /ipcRenderer\.invoke\('updater:cancel-download'\)/u)
...
test('主进程应初始化自动更新、静默检查并注册完整 IPC', () => {
  ...
  assert.match(mainSource, /ipcMain\.handle\('updater:cancel-download'/u)
...
test('设置页应提供自适应且支持主题的版本检查、进度和安装操作', () => {
  ...
  assert.match(html, /id="update-cancel"/u)
  ...
  assert.match(source, /window\.api\.cancelUpdateDownload\(\)/u)
  assert.match(source, /updateCancelButton\.hidden = status\.phase !== 'downloading'/u)
