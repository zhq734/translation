...
test('同一段落的多个文本单元应合并为一个请求，跨段落仍分别去重入队', async () => {
  const coordinator = new PageTranslationCoordinator({
    translate: async (text) => ({ translation: text })
  })
  ...
  const result = stream.enqueue([
    { ...base, id: 'same-1', anchor: { ...base.anchor, shadowPath: [0, 1] } },
    { ...base, id: 'same-2', anchor: { ...base.anchor, shadowPath: [0, 2] } },
    { ...base, id: 'other-1', blockId: 'block-other', anchor: { ...base.anchor, semanticAttribute: 'placeholder' } },
    { ...base, id: 'other-2', blockId: 'block-other', anchor: { ...base.anchor, semanticAttribute: 'aria-label' } }
  ])
  stream.closeInput()
  await stream.result

  assert.deepEqual(result, { accepted: 2, duplicate: 0, truncated: 0 })
})
