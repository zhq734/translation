...
export class SelectionCaptureCoordinator {
  ...
  /**
   * 在显示“译”按钮前捕获当前选中文字，仅缓存非空且无错误的有效结果。
   * @param anchor 当前选区右上角锚点。
   * @returns 当前请求的捕获结果；如果请求已被更新则返回 null。
   * @author zhenghq
   */
  prepare(anchor: SelectionAnchor): Promise<SelectionCaptureResult | null> {
    this.preparedSelection = null
    return this.enqueue(anchor, true)
  }
  ...
  capture(anchor?: SelectionAnchor): Promise<SelectionCaptureResult | null> {
    this.preparedSelection = null
    return this.enqueue(anchor, false)
  }

  /**
   * 消费按钮显示前已经缓存的选中文字，避免点击按钮后选区失效。
   ...
   * @param prepare 是否把有效结果保存为按钮点击时使用的缓存。
  ...
  private enqueue(
    ...
    const task = this.captureChain
      ...
      .then(async () => {
        ...
        if (prepare && result.text && !result.error) this.preparedSelection = result
        return result
      })
