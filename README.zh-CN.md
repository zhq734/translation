...
- **微软翻译**：支持 Azure Translator 订阅密钥、可选 Region、在线检测和加密凭证存储。
...
<p align="center">
  <img src="./docs/images/截图3.png" alt="设置页面" width="480">
</p>
...
| 钉钉翻译 | 企业内部应用翻译通道 | 关闭 |
| 微软翻译 | Azure 订阅密钥和可选资源 Region | 关闭 |
...
钉钉 `ClientSecret` 与微软订阅密钥会分别存放在同一个 `userData` 目录下：
...
| 2 | 微软翻译 | 已启用、已配置订阅密钥、语言对受支持 | Azure AI Translator Text v3；全局单服务资源可不填 Region |
...
- 钉钉或微软翻译未启用、配置不完整、语言对不支持时，不会产生对应网络请求。
...
微软翻译通道使用 Azure AI Translator Text v3 REST API。

1. 创建或选择一个 Azure Translator 资源；
2. 在资源的“密钥和终结点”页面复制订阅密钥；
3. 打开“划词翻译 → 设置…”，启用“微软翻译”；
4. 填写订阅密钥；
5. 区域资源或多服务资源需要填写对应 Region；全局单服务 Translator 资源可留空；
6. 点击“保存微软配置”，再点击“检测配置”，验证鉴权和文本翻译链路。

安全行为：

- 订阅密钥只通过设置页提交给 Electron 主进程；
- 使用 Electron `safeStorage` 加密后单独写入 `microsoft-credentials.json`；
- 普通 `settings.json`、渲染进程设置快照、状态提示和翻译错误不会包含明文密钥；
- 订阅密钥输入框留空并保存其他字段时，会保留旧凭证；
- 如需删除凭证，必须点击“清除密钥”；
- 配置变化后会清理微软相关翻译缓存和熔断状态；
- 鉴权、权限、限流、参数、服务或网络错误会经过脱敏，并自动降级到后续通道。
...
本项目不提供云端账户系统。钉钉和微软凭证仅在对应通道启用并完成配置后使用；公开设置只保存非敏感字段和“凭证是否已配置”的状态，明文凭证仅存在于独立的 `safeStorage` 加密文件中。
...
│   │   ├── microsoftConfig.ts        # 微软公开配置编排
│   │   ├── microsoftCredentials.ts   # 微软订阅密钥加密存储
│   │   ├── microsoftErrors.ts        # 微软错误分类与脱敏
│   │   ├── microsoftLanguage.ts      # 微软语言代码适配
│   │   └── microsoftTranslation.ts   # Translator Text v3 请求适配器
...
- 微软语言适配、Translator v3 请求、错误分类、缓存重置和通道降级；
- 微软订阅密钥加密存储、回滚、清除和脱敏；
- 设置页钉钉与微软配置 UI 契约。
...
- 确认系统钥匙串/安全存储可用；
- 检查订阅密钥，以及资源要求填写时对应的 Azure Region；
- 只有全局单服务 Translator 资源才应留空 Region；
- 不要把订阅密钥写入 `settings.json`，请使用设置页的微软专用配置入口。
...
- 翻译请求依赖网络和第三方服务，钉钉、微软翻译、自建/公共 DeepLX、Google 和 MyMemory 均可能受上游变更影响；
