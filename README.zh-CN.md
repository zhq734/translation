...
- **本机语音朗读**：点击弹窗中的扬声器图标即可朗读当前有效译文，再次点击可停止；不会自动播放，不需要云端 TTS、API Key 或额外费用。应用会为每种语言固定选择一个稳定的首选系统音色，不在普通、增强或男女声音色之间动态切换；中文优先使用更自然的 `Xiaoxiao`，macOS 未安装该音色时优先使用普通话音色 `Meijia`，最后才回退到 `Tingting`。英文会避开已知容易出现失真或音量异常的实验音色，实际音质取决于操作系统已安装的语音。
...
- **朗读译文**：点击扬声器图标使用本机系统语音朗读当前译文，再次点击停止；译文不会自动播放；
- **图钉**：固定或取消固定弹窗；
- **复制**：将当前译文写入系统剪贴板；
