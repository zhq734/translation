...
划词取词依赖 macOS 的"辅助功能"权限。应用会优先通过辅助功能接口（`AXSelectedText`）直读当前前台应用的选中文字，直读不可用或为空时才回退到受控的 `Command+C` 模拟复制，因此首次使用前必须授权。常驻原生 helper（`macos-ax-reader`）通过辅助功能 API 执行读取，不启动 AppleScript 或 System Events，因此**仅需辅助功能权限**，不再触发 Automation 授权弹窗。
...
- **Windows**：通过 UI Automation（`TextPattern.GetText`）读取焦点控件的选区；若应用不支持 `TextPattern`（如部分传统 Win32 控件、PDF 阅读器），则回退到受控 `Ctrl+C` 并读取剪贴板。
  常驻 `windows-uia-reader` helper 通过 stdio 行分隔 JSON 通信。当 `TextPattern` 不可用时，helper 先向焦点控件发送 `WM_COPY` 消息（500ms 超时），再回退到全局 `Ctrl+C` 快捷键，因此按钮触发取词时不会干扰用户的按键状态。预编译的 `windows-uia-reader.exe` 随安装包分发，其 SHA-256 校验值见 `helpers/windows-uia-reader/README.md`，可据此验证文件完整性。
...
5. 在“译文 / 原文 / 对照”之间切换时无需刷新页面，切回译文会复用当前翻译结果；对照模式按段落把译文追加在原文下方，原文淡化到 60% 不透明度，不重建页面结构；
...
| 并发请求数 | 同时向翻译通道发起的请求数量，1~8，调高可加快长网页翻译 | 3 |
| 默认显示 | 译文、原文或对照 | 译文 |
| 图片文字识别 | 对页面图片执行 OCR，并把识别出的文字翻译后展示 | 开启 |
| 单页最大图片数 | 单页最多处理多少张候选图片，超出部分计入跳过 | 30 |
| 最小图片边长 | 过滤图标、头像、跟踪像素等微小图片的边长阈值（像素） | 64 |
| 图片译文展示 | 在图片下方显示说明块，或在图片区域内叠加半透明译文 | 图片下方 |

长段落会优先按句子边界拆分，必要时再按安全字符边界切分；翻译通道不会静默截断原文并伪装成功。网页超过块数/字数上限或个别文本单元失败时，顶部状态栏会明确提示“仅翻译了部分网页内容”。已完成的网页翻译会通过有界内存 LRU 缓存复用：页面缓存最多保存 30 个页面、每页最多 3 个语言方向；通用翻译结果缓存最多保存 50,000 条或约 50 MB。上述缓存只存在于内存中，不会写入磁盘。取消、导航、重新翻译、切换语言或关闭阅读器都会让旧任务失效，迟到的通道结果不会覆盖当前页面。原位写回只修改匹配的文本节点值，保留 DOM 结构、链接、按钮、事件、滚动位置和首尾空白；锚点失配时跳过写回并提示页面内容已更新。对照模式通过受控注入节点呈现译文，提取脚本与增量收集器会跳过这些节点，不会把译文当作新原文再次翻译；只有块内全部文本单元都有成功译文时才渲染该段对照译文，失败或锚点失配的块保持原文并单独提示补译。按钮、导航、表单语义提示与短碎片在对照模式下保持原文、不注入译文，以避免撑开布局；这些文本仍照常翻译并缓存，切换到“译文”模式即可立即看到译文。开放 Shadow DOM 内的文本块不支持对照渲染，会按设计跳过并单独统计，不计入失败。

开启“图片文字识别”后，点击“翻译此页”会在提取文本的同时收集可见图片候选（`img`、`picture`、`canvas` 以及带 `background-image` 且尺寸达标的元素），过滤隐藏图片、装饰图和小于“最小图片边长”的图片，最多处理“单页最大图片数”张。取图优先使用阅读器独立 Session 按页面 Cookie 请求图片地址；当请求失败、跨域受限或来源为 `canvas`、`data:`、`blob:` 内联资源时，回退为图片区域的页面截图。取图受单图最大字节（8 MB）与最大像素（1,600 万）保护，超限图片按跳过处理，不会中断文本翻译。

图片字节只在本机交给现有 OCR 引擎处理，复用 OCR 语言、引擎偏好、自适应预处理与质量门禁；识别为空、判定为噪声或质量不达标时不会进入翻译，也不注入译文。图片译文默认在图片下方以说明块展示，选择“叠加在图片上”后在图片区域内以半透明层展示；覆盖节点使用主题变量，不改写图片字节，也不移除或替换原图元素。图片 OCR 与译文结果按页面指纹、图片指纹和语言方向进入与文本隔离的页面缓存，同页同语言方向再次翻译时直接复用，不重复取图与 OCR；失败或取消的结果不写入缓存。状态栏会独立显示图片已处理/候选数、跳过数和失败数，图片失败只按“部分翻译”提示，不影响文本翻译完成。

网页翻译的翻译请求默认按 3 路并发执行，可在设置页“并发请求数”中调整为 1~8。页面写回与网络请求解耦：某个文本单元翻译完成后立即进入原位写回队列，慢速的 DOM 写回不会占用网络并发槽位，因此后续段落仍会持续发起翻译请求。调高并发会同时增加翻译服务商的瞬时压力，可能触发限流或熔断，建议在自建或高配额通道下使用较高数值。
...
网页全文翻译还遵循额外的显式授权边界：应用无法读取系统浏览器标签页；内置阅读器仅处于打开状态时不会提取内容；只有用户点击"翻译此页"后，应用才读取当前主文档的可见 DOM 与可见图片。阅读器使用独立的持久化 Session，与翻译 API Session 的 Cookie 和缓存隔离；动态页面变化只在本地提示重新翻译，不会自动提取或上传新增文本；进入翻译范围的网页文本，以及图片 OCR 识别出的文字，仍会发送到设置中启用的翻译服务；图片字节本身只在本机处理，不会作为图片上传。

### 划词诊断

应用会在本地记录每次取词的诊断元数据，用于排查"偶现取不到词"问题。诊断记录仅包含：时间戳、入口（按钮/快捷键/自动）、平台、命中级别（原生直读/复制轮询/稳定期命中/失败）、失败原因、耗时和前台应用标识。**绝不记录选中文字、剪贴板内容、窗口标题、文件路径或文本长度。** 数据保存在本地 `userData/capture-diagnostics.json`，按两天滚动清理，不会发送到任何远端服务器。可在设置页"日志"区域查看摘要，并导出为 JSON 文件用于反馈。
